...
async function ppRequestNotificationPermission() {
...
async function ppRegisterServiceWorker() {
...
async function ppSubscribeToPush() {
    const token = localStorage.getItem('auth_token');
    if (!token) return;
    
    try {
        const permission = await ppRequestNotificationPermission();
        if (!permission) return;
        
        const registration = await ppRegisterServiceWorker();
        ...
        if (subscription) {
            // Send to server
            await ppSendSubscriptionToServer(subscription);
            return;
        }
        ...
        await ppSendSubscriptionToServer(newSubscription);
...
async function ppSendSubscriptionToServer(subscription) {
    const token = localStorage.getItem('auth_token');
    if (!token) return;
...
async function ppInitPushNotifications() {
    ...
    const settingsEnabled = await fetch('/api/notifications/preference.php', {
        headers: { 'Authorization': 'Bearer ' + token }
    }).then(res => res.json()).then(data => data.enabled !== false);
    
    if (settingsEnabled) {
        await ppSubscribeToPush();
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    const token = localStorage.getItem('auth_token');
    if (token) {
        setTimeout(ppInitPushNotifications, 2000);
    }
});
